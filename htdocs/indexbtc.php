				...
				switch($k) {
					case "with_any_one_of":
						...
						for($i=0;$i<$wordsCount;$i++) {
							if(!empty($_POST["search"]["search_in"])) {
								...
								$queryCondition .= "compressed LIKE '" . $wordsAry[$i] . "%' OR uncompressed LIKE '" . $wordsAry[$i] . "%'";
							}
							if($i!=$wordsCount-1) {
								$queryCondition .= " OR ";
							}
					...
					case "with_the_exact_of":
						...
						if(!empty($_POST["search"]["search_in"])) {
							...
							$queryCondition .= "compressed LIKE '%" . $v . "%' OR uncompressed LIKE '%" . $v . "%'";
						}
						break;
					case "without":
						$without = $v;
						if(!empty($_POST["search"]["search_in"])) {
							...
							$queryCondition .= "compressed NOT LIKE '%" . $v . "%' AND uncompressed NOT LIKE '%" . $v . "%'";
						}
						break;
					case "starts_with":
						$starts_with = $v;
...
<html>
				...
				<div id="cont">
					<div id="terminal">
					  <div id="terminal2">
						<div>
								<div id="headtext">
									<h1 style="text-align:center; color:black; font-size:25px;"><img src="https://www.mizogg.co.uk/Images/btcimage.png" width="50" height="50" alt=""/>Advanced Search using Bitcoin Address<img src="https://www.mizogg.co.uk/Images/btcimage.png" width="50" height="50" alt=""/></h1>
										<div id="MizLogo" style="text-align:center; color:black; font-size:16px;">
										<div>Total Bitcoin Wallets Included ??????????. Good Luck Hunting From</div>
	...
	<body>
		<h1>CryptoCrackers Advanced BTC Search</h1>
    <div>      
			<form name="frmSearch" method="post" action="indexbtc.php">
			...
			<div class="search-box">
				...
				<div>
					<input type="text" name="search[with_any_one_of]" class="BitcoinInputBox" value="<?php echo $with_any_one_of; ?>"	/>
				...
				<div id="advanced-search-box" <?php if(empty($advance_search_submit)) { ?>style="display:none;"<?php } ?>>
					...
					<div>
						<input type="text" name="search[with_the_exact_of]" id="with_the_exact_of" class="BitcoinInputBox" value="<?php echo $with_the_exact_of; ?>"	/>
					</div>
					<label class="search-label">Without:</label>
					<div>
						<input type="text" name="search[without]" id="without" class="BitcoinInputBox" value="<?php echo $without; ?>"	/>
					</div>
					<label class="search-label">Starts With:</label>
					<div>
						<input type="text" name="search[starts_with]" id="starts_with" class="BitcoinInputBox" value="<?php echo $starts_with; ?>"	/>
					</div>
					<label class="search-label">Search Keywords in:</label>
					<div>
						<select name="search[search_in]" id="search_in" class="BitcoinInputBox">
							<option value="">Select Column</option>
							<option value="compressed" <?php if($search_in=="compressed") { echo "selected"; } ?>>compressed</option>
							<option value="uncompressed" <?php if($search_in=="uncompressed") { echo "selected"; } ?>>uncompressed</option>
						</select>
					</div>
				</div>
...
while($row = mysqli_fetch_assoc($result)) {
	...
	$compresssed = $row["compressed"];
	$uncompresssed = $row["uncompressed"];
...
?>
<div></br>
				<div><strong>Privatekey DEC : <?php echo $row["seed"]; ?></strong></div>
				<div><strong>Privatekey HEX : <?php echo $row["privatekey"]; ?></strong></div>
				<div><strong>Compressed : <?php echo $compresssed; ?></strong> <strong><div style="color:green"><?php echo ' Balance : '. $r->balance; ?> Sats</strong><strong><?php echo ' TotalReceived : '. $r->totalReceived; ?> Sats </strong><strong><?php echo ' TotalSent : '. $r->totalSent; ?> Sats </strong><strong><?php echo ' Transactions : '. $r->txs; ?></strong></div></div>
				<div><strong>Uncompressed : <?php echo $uncompresssed; ?></strong> <strong><div style="color:green"><?php echo ' Balance : '. $rr->balance; ?> Sats</strong><strong><?php echo ' TotalReceived : '. $rr->totalReceived; ?> Sats </strong><strong><?php echo ' TotalSent : '. $rr->totalSent; ?> Sats </strong><strong><?php echo ' Transactions : '. $rr->txs; ?></strong></div>
